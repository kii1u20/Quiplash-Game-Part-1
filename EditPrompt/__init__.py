...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        prompt = list(prompts_container.query_items(query= "SELECT * FROM c WHERE c.id = '{0}'".format(id), enable_cross_partition_query = True))
        if len(user_exists) == 0 :
            ...
            if len(text) < 20 or len(text) > 100:
                ...
                prompt[0]['text'] = text
                prompts_container.upsert_item(body=prompt[0])
