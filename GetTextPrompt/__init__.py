...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    word = input["word"]
    if c == True:
        sensitive = list(prompts_container.query_items(query="SELECT c.id, c.text, c.username FROM c WHERE c.text LIKE '%[^a-z]{0}[^a-z]%' OR c.text LIKE '{0}[^a-z]%' OR c.text LIKE '%[^a-z]{0}'".format(word), enable_cross_partition_query=True))
        return func.HttpResponse(body=json.dumps(sensitive))
    else:
        insensitive = list(prompts_container.query_items(query="SELECT c.id, c.text, c.username FROM c WHERE LOWER(c.text) LIKE LOWER('%[^a-z]{0}[^a-z]%') OR LOWER(c.text) LIKE LOWER('{0}[^a-z]%') OR LOWER(c.text) LIKE LOWER('%[^a-z]{0}')".format(word), enable_cross_partition_query=True))
